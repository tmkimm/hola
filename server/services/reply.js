export class ReplyService {
    ...
    async registerReply(userID, comment) {
        ...
        const study = await this.studyModel.registerReply(studyId, commentId, content, userID);

        // 대댓글 등록 시 댓글 등록자에게 달림 추가
        let author = await this.studyModel.findAuthorByCommentId(commentId);
        await this.notificationModel.registerNotification(studyId, author, userID, 'reply');        // 알림 등록
    ...
    async modifyReply(comment, tokenUserId) {
        await this.studyModel.chkeckReplyAuthorization(comment.id, tokenUserId);
    ...
    async deleteReply(replyId, userId) {
        await this.studyModel.chkeckReplyAuthorization(replyId, userId);
        let author = await this.studyModel.findAuthorByReplyId(replyId);
        const studyRecord = await this.studyModel.deleteReply(replyId);
        await this.notificationModel.deleteNotification(studyRecord._id, author, userId, 'reply');   // 알림 삭제
    }
}